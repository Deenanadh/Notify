...
Item _itemForMessage(Map<String, dynamic> message) {
  ...
  final Item item = _items.putIfAbsent(itemId, () => Item(itemId: itemId, itemBody: itemBody))
    ..status = data['status'];
  return item;
}
...
class _HomePageState extends State<HomePage> {
  ...
  void showNotification() async {
    ...
    await fltNotification.show(0, 'Notify', 'Alert', generalNotificationDetails,
            ...
            if (_pinChangeKey.currentState.validate()) {
              if (_newPinKey.currentState.validate()) {
                await FirebaseFirestore.instance
                    .collection('users')
                    .doc(FirebaseAuth.instance.currentUser.uid.toString())
                    .update({'pin': this.newPin}).then((value) {
                  setState(() {
                    userPin = this.newPin;
                  });
