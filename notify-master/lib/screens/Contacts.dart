...
class _contactsState extends State<contacts> {
  ...
  void initState() {
    ...
    userPhoneNumberfull =
        FirebaseAuth.instance.currentUser.phoneNumber.toString().replaceAll(' ', "replace");
    userPhoneNumber =
        userPhoneNumberfull.substring(userPhoneNumberfull.length - 10, userPhoneNumberfull.length);
    super.initState();
    // _query();
  }
  ...
  Future<void> getContacts(String number) async {
    ...
    String recievedtrimmedContact =
        recievedfullContact.substring(recievedfullContact.length - 10, recievedfullContact.length);
    ...
    await FirebaseFirestore.instance.collection('users').get().then((value) {
      ...
      value.docs.forEach((element) {
        ...
        userPhoneNumberTrimmed = userPhoneNumberFull.substring(
            userPhoneNumberFull.length - 10, userPhoneNumberFull.length);
        ...
        if (trimmedContact == userPhoneNumberTrimmed) {
          ...
          setState(() {
            ...
            userNumber =
                data['phoneNumber'].substring(userNumberFull.length - 10, userNumberFull.length);
            name = data['name'];
            uid = data['id'];
            loop = true;
          });
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              if (requestsData[index]['accepted'] == 'false') {
                                ...
                                String userPhoneFull =
                                    FirebaseAuth.instance.currentUser.phoneNumber.toString();
                                String userPhone = userPhoneFull.substring(
                                    userPhoneFull.length - 10, userPhoneFull.length);
                                FirebaseFirestore.instance.collection('users').doc(userID).update({
                                  'requests': FieldValue.arrayRemove([
                                    {
                                      'uid': requestsData[index]['uid'],
                            ...
                            child: requestsData[index]['accepted'] == 'false'
                                ? Icon(Icons.check)
                                : Icon(Icons.delete),
                          ),
                        ),
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return ListTile(
                        ...
                        leading: (sentRequestsData[index] != null)
                            ...
                            : CircleAvatar(
                                child: Text(
                                    sentRequestsData[index]['name'].toString().substring(0, 1)),
                                backgroundColor: Theme.of(context).accentColor,
                              ),
                        trailing: Container(
                          height: 30,
                          ...
                          child: ElevatedButton(
                            ...
                            child: sentRequestsData[index]['accepted'] == 'false'
